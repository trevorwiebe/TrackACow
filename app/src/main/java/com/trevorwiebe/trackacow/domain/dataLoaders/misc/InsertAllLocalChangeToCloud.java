        ...
        try {
            ...
            for (int h = 0; h < holdingLoadEntities.size(); h++) {
                ...
                LoadEntity loadEntity = new LoadEntity(
                        0,
                        cacheLoadEntity.getNumberOfHead(),
                        cacheLoadEntity.getDate(),
                        cacheLoadEntity.getDescription(),
                        cacheLoadEntity.getLotId(),
                        cacheLoadEntity.getLoadId()
                );
                switch (cacheLoadEntity.getWhatHappened()) {
                    case Constants.INSERT_UPDATE:
                        baseRef.child(Constants.LOAD).child(loadEntity.getLoadId()).setValue(loadEntity);
                        break;
                    case Constants.DELETE:
                        baseRef.child(Constants.LOAD).child(loadEntity.getLoadId()).removeValue();
                        break;
                }
            }
...
//            db.holdingCallDao().deleteCallTable();
            ...
            for (int g = 0; g < holdingUserEntities.size(); g++) {
                ...
                UserEntity userEntity = new UserEntity(
                        0,
                        cacheUserEntity.getAccountType(),
                        cacheUserEntity.getDateCreated(),
                        cacheUserEntity.getName(),
                        cacheUserEntity.getEmail(),
                        cacheUserEntity.getRenewalDate(),
                        cacheUserEntity.getUid()
                );
                switch (cacheUserEntity.getWhatHappened()) {
                    case Constants.INSERT_UPDATE:
                        baseRef.child(Constants.USER).setValue(userEntity);
                        break;
                    case Constants.DELETE:
                        baseRef.child(Constants.USER).removeValue();
                        break;
                }
            }
